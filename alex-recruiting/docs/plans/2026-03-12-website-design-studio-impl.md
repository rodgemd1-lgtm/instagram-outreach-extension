# Website Design Studio — Implementation Plan

> **For Claude:** REQUIRED SUB-SKILL: Use superpowers:executing-plans to implement this plan task-by-task.

**Goal:** Build a unified `website-design-studio` skill that orchestrates 6-phase website design (Research → Strategy → Design System → Layout → Build → Verify) with checkpoint gates, integrating Susan agents, MCP tools, and the frontend-design skill.

**Architecture:** A single skill file (`.claude/skills/website-design-studio/SKILL.md`) that serves as a pipeline orchestrator. Each phase dispatches parallel subagents where possible, produces artifacts saved to `docs/studio/{project}/`, and pauses for user checkpoint approval before advancing. No application code — this is a Claude Code skill definition.

**Tech Stack:** Claude Code skills (markdown), Susan Intelligence MCP, Firecrawl MCP (branding extraction), Exa MCP (semantic search), Context7 MCP, preview tools
...
**Files:**
- Create: `alex-recruiting/.claude/skills/website-design-studio/SKILL.md`

**Step 1: Create the directory**

```bash
mkdir -p /Users/mikerodgers/Desktop/alex-recruiting-project/alex-recruiting/.claude/skills/website-design-studio
```

**Step 2: Write the skill frontmatter and overview**
...
```bash
git add alex-recruiting/.claude/skills/website-design-studio/SKILL.md
git commit -m "feat(studio): scaffold website-design-studio skill"
```

---

## Task 2: Write Phase 1 — Research Engine

**Files:**
- Modify: `alex-recruiting/.claude/skills/website-design-studio/SKILL.md`

**Step 1: Append Phase 1 to the skill file**

Add after the pipeline overview:
...
```bash
git add alex-recruiting/.claude/skills/website-design-studio/SKILL.md
git commit -m "feat(studio): add Phase 1 research engine"
```

---

## Task 3: Write Phase 2 — Strategy
...
```bash
git add alex-recruiting/.claude/skills/website-design-studio/SKILL.md
git commit -m "feat(studio): add Phase 2 strategy synthesis"
```

---

## Task 4: Write Phase 3 — Design System Generator
...
```bash
git add alex-recruiting/.claude/skills/website-design-studio/SKILL.md
git commit -m "feat(studio): add Phase 3 design system generator"
```

---

## Task 5: Write Phase 4 — Layout Architecture
...
```bash
git add alex-recruiting/.claude/skills/website-design-studio/SKILL.md
git commit -m "feat(studio): add Phase 4 layout architecture"
```

---

## Task 6: Write Phase 5 — Build
...
```bash
git add alex-recruiting/.claude/skills/website-design-studio/SKILL.md
git commit -m "feat(studio): add Phase 5 build with frontend-design integration"
```

---

## Task 7: Write Phase 6 — Verify
...
```bash
git add alex-recruiting/.claude/skills/website-design-studio/SKILL.md
git commit -m "feat(studio): add Phase 6 verification pipeline"
```

---

## Task 8: Add Studio Artifact Templates

**Files:**
- Create: `alex-recruiting/docs/studio/.gitkeep`

**Step 1: Create the studio directory**

```bash
mkdir -p /Users/mikerodgers/Desktop/alex-recruiting-project/alex-recruiting/docs/studio
touch /Users/mikerodgers/Desktop/alex-recruiting-project/alex-recruiting/docs/studio/.gitkeep
```

**Step 2: Commit**

```bash
git add alex-recruiting/docs/studio/.gitkeep
git commit -m "feat(studio): add docs/studio directory for pipeline artifacts"
```

---

## Task 9: Verify Skill Loads Correctly

**Step 1: Check skill is discoverable**

```bash
ls -la /Users/mikerodgers/Desktop/alex-recruiting-project/alex-recruiting/.claude/skills/website-design-studio/SKILL.md
```

Verify file exists and is readable.

**Step 2: Read the complete skill file to verify coherence**

Read the full `SKILL.md` and verify:
- All 6 phases are present
- All checkpoints reference user approval
- All agent references use correct `subagent_type` or `mcp__susan-intelligence__run_agent` syntax
- All artifact paths use `docs/studio/{project-name}/` pattern
- No broken references or placeholder text

**Step 3: Commit final version if any fixes needed**

```bash
git add alex-recruiting/.claude/skills/website-design-studio/SKILL.md
git commit -m "fix(studio): polish skill file for coherence"
```

---

## Task 10: Apply Studio to Alex Recruiting V3.0

This is the first real application of the studio. Invoke the `website-design-studio` skill with:
- **Project type:** recruiting
- **Project name:** recruit-v3
- **Existing URL:** https://alex-recruiting.vercel.app/recruit
- **Optimization goal:** Design excellence

Run the full 6-phase pipeline with checkpoint gates, producing artifacts in `docs/studio/recruit-v3/`.

This task is large and will be guided by the skill itself — each phase will pause for user checkpoint approval.

---

## Execution Notes

- Tasks 1-9 are the studio build (skill authoring, ~30 min)
- Task 10 is the V3.0 application (full pipeline, user-guided with checkpoints)
- The skill is a markdown file — no application code to test with TDD
- Verification is reading the skill and confirming it references the right tools/agents
- After Task 9, the studio is ready for use on any project
