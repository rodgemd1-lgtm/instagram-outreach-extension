  ...
  try {
    ...
    if (dbUrl && dbUrl !== "your_postgresql_connection_string_here") {
      try {
        ...
        console.error("[analytics/visit] DB insert failed, using generated ID:", dbErr);
        visitId = randomUUID();
      }
    } else {
    ...
    console.error("[analytics/visit] Unhandled error:", error);
    return NextResponse.json({ error: "Failed to record page visit", details }, { status: 500 });
  }
}
