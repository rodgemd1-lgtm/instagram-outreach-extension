...
import { MINNESOTA_D3_OUTREACH } from "@/lib/data/minnesota-d3-outreach";
...
// ---------------------------------------------------------------------------
// POST /api/outreach/seed-minnesota-d3 — Seed Minnesota D3 outreach campaign
...
export async function POST(request: NextRequest) {
  ...
  for (const coach of MINNESOTA_D3_OUTREACH) {
    ...
    try {
      ...
      if (isSupabaseConfigured()) {
        try {
          ...
          const { error } = await supabase.from("dm_messages").insert({
            ...
            coach_name: coach.coachName,
            school_name: coach.schoolName,
            template_type: "minnesota_d3_outreach",
      ...
      if (!dmInserted) {
        insertDM({
          ...
          coachName: coach.coachName,
          schoolName: coach.schoolName,
          templateType: "minnesota_d3_outreach",
          content: coach.dmDraft,
          status: "drafted",
          sentAt: null,
          respondedAt: null,
  ...
  return NextResponse.json(
    {
      message: `Minnesota D3 campaign seeded: ${summary.emailsCreated} emails, ${summary.dmsCreated} DMs, ${summary.postsCreated} X posts`,
      summary,
      details: results,
    },
    { status: 201 }
