...
// ---------------------------------------------------------------------------
// POST /api/outreach/seed-iowa-d3 — Seed Iowa D3 outreach campaign
...
export async function POST() {
  ...
  for (const coach of IOWA_D3_OUTREACH) {
    ...
    try {
      const post: Omit<Post, "id" | "createdAt" | "updatedAt"> = {
        ...
        hashtags: ["D3Football", "RecruitJacob", "IowaD3"],
        mediaUrl: null,
        scheduledFor: "",
      ...
      if (isSupabaseConfigured()) {
        try {
          ...
          const { error } = await supabase.from("dm_messages").insert({
            ...
            coach_name: coach.coachName,
            school_name: coach.schoolName,
            template_type: "iowa_d3_outreach",
  ...
  return NextResponse.json(
    {
      message: `Iowa D3 campaign seeded: ${summary.emailsCreated} emails, ${summary.dmsCreated} DMs, ${summary.postsCreated} X posts`,
      summary,
      details: results,
    },
    { status: 201 }
