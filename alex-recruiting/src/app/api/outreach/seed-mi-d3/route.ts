...
import { MI_D3_OUTREACH } from "@/lib/data/mi-d3-outreach";
...
// ---------------------------------------------------------------------------
// POST /api/outreach/seed-mi-d3 — Seed Michigan D3 outreach campaign
// Creates email drafts, DM drafts, and X callout posts for MIAA programs
// ---------------------------------------------------------------------------
...
export async function POST(request: NextRequest) {
  ...
  // Optional: filter by wave number via query param ?wave=1
  const { searchParams } = new URL(request.url);
  const waveFilter = searchParams.get("wave") ? parseInt(searchParams.get("wave")!) : null;

  const coaches = waveFilter
    ? MI_D3_OUTREACH.filter((c) => c.wave === waveFilter)
    : MI_D3_OUTREACH;
        ...
        try {
          ...
          if (existing && existing.length > 0) {
            ...
            const { error } = await supabase.from("dms").insert({
              coach_name: coach.coachName,
              school_name: coach.schoolName,
              coach_handle: coach.xHandle ?? null,
