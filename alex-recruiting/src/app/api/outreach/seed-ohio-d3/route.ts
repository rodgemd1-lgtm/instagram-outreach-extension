...
import { OHIO_D3_OUTREACH } from "@/lib/data/ohio-d3-outreach";
...
// ---------------------------------------------------------------------------
// POST /api/outreach/seed-ohio-d3 — Seed Ohio D3 outreach campaign
...
export async function POST(request: NextRequest) {
  ...
  for (const coach of OHIO_D3_OUTREACH) {
    ...
    try {
      const post: Omit<Post, "id" | "createdAt" | "updatedAt"> = {
        ...
        hashtags: ["D3Football", "RecruitJacob", "OhioD3"],
        mediaUrl: null,
        scheduledFor: "",
      ...
      if (isSupabaseConfigured()) {
        try {
          ...
          const { error } = await supabase.from("dm_messages").insert({
            ...
            coach_name: coach.coachName,
            school_name: coach.schoolName,
            template_type: "ohio_d3_outreach",
  ...
  return NextResponse.json(
    {
      message: `Ohio D3 campaign seeded: ${summary.emailsCreated} emails, ${summary.dmsCreated} DMs, ${summary.postsCreated} X posts`,
      summary,
      details: results,
    },
    { status: 201 }
